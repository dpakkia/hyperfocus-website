import { ssrRenderAttr, ssrRenderStyle, ssrInterpolate } from "vue/server-renderer";
...
const _sfc_main = {
  ...
  setup(__props) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      _push(`<!--[--><div class="relative w-screen h-screen flex justify-center items-start bg-[#333] overflow-hidden"><img${ssrRenderAttr("src", _imports_0)} alt="Background Smoke" class="absolute inset-0 w-full h-full object-cover z-0"><div class="mt-[80vh] w-[430px] h-[14px] bg-white z-20"></div><div class="absolute bg-white w-[14px] h-[140px] z-[21]" style="${ssrRenderStyle({
        bottom: "calc(20vh)",
        ...
        left: "50%",
        transform: "translateX(-50%)"
      })}"><span class="">SCOPRI I </span><span class="text-[32px] font-medium">NOSTRI SERVIZI</span></div><img${ssrRenderAttr("src", _imports_5)} alt="Scroll Down Icon" class="absolute w-[32px] h-auto z-30 animate-bounce" style="${ssrRenderStyle({
        bottom: "10vh",
        left: "50%",
        transform: "translateX(-50%)"
      })}"></div><div class="fixed top-5 left-5 bg-black/70 text-white text-sm p-2 rounded z-50 font-mono"> Scroll: ${ssrInterpolate(scrollPercent.value.toFixed(1))}%<br> ${ssrInterpolate(scrollPercent.value > 25 ? "✅ Superato 25%" : "⬆️ Sotto 25%")}</div><div class="w-full h-[100vh]"></div><!--]-->`);
    };
  }
};
...
//# sourceMappingURL=test-cZtNC6uT.js.map
